...
bool CLuaH::loadFilesDequeStorage(const std::string &path, scriptStorage &storage)
{
	...
	if (direntd)
	{
		...
		for (auto &fname : tmp)
		{
			CLog::log() << ("Loading <<" + path + "/" + fname + ">>");
			storage.push_back(newScript(path, fname));
		}
		closedir(direntd);
	}
...
CLuaH::luaScript CLuaH::newScriptR(const std::string &memf, const std::string &name){
	...
	if (load_result != 0){
		CLog::log() << std::string("luaL_loadbuffer(") + std::to_string((uintptr_t)lData.luaState) +
...
CLuaH::luaScript CLuaH::newScript(const std::string &path, const std::string &f)
{
	...
	if (load_result != 0){
		CLog::log() << std::string("luaL_loadfile(") + std::to_string((uintptr_t)lData.luaState) +
			std::string(", ") + std::string(lData.filePath + barra + lData.fileName) +
			std::string(") failed to load with result ") + std::to_string(load_result);
...
bool CLuaH::runInternalRouteWithParams(luaScript &L, size_t id, const multiCallBackParams_t &params)
{
	if (L.callbacksAdded)
	{
		auto it = L.routes.find(id);

		if (it != L.routes.end() && (*it).second != 0)
		{
			lua_rawgeti(L.luaState, LUA_REGISTRYINDEX, (*it).second);
...
CLuaH::luaScript &CLuaH::luaScript::operator=(luaScript &&script){
	...
	routes = std::move(script.routes);
	customPtr = script.customPtr;
...
CLuaH::luaScript CLuaH::luaScript::clone()
{
	...
	//lData.luaState->gclist = luaState->gclist;
	//lData.luaState->status = luaState->status;
	//lData.luaState->hook = luaState->hook;
	//lData.luaState->l_G = luaState->l_G;
...
std::vector<char> CLuaH::luaScript::dumpBytecode()
{
	...
	if (dump_result != 0)
	{
		CLog::log() << std::string("lua_dump(") + std::to_string((uintptr_t)luaState) +
